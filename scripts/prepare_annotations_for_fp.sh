...
score_save_name=k600_val_step${frames_step}_scorer_${token_select_mode}_tok${token_select_num}_b${bs_per_gpu}_$(date +"%Y-%m-%d_%H-%M-%S").pt
...
torchrun --nnodes=${NNODES} \
         ...
         --master_port=${MASTER_PORT} \
            ...
            --csv_file k600_val.csv --out_path work_dir/adaptok/ \
            ...
            model.args.mode get_ar_fp_annotations \
...
bs_per_gpu=128
tot_bs=$((bs_per_gpu * NNODES * GPUS_PER_NODE))
score_save_name=k600_train_step${frames_step}_scorer_${token_select_mode}_tok${token_select_num}_b${bs_per_gpu}_$(date +"%Y-%m-%d_%H-%M-%S").pt
...
torchrun --nnodes=${NNODES} \
         ...
         --master_port=${MASTER_PORT} \
            train.py --cfg cfgs/adaptok_scorer.yaml \
            --manualSeed 66667 --tag adaptok_scorer_t2048 \
            --csv_file k600_train.csv --out_path work_dir/adaptok/ \
            --name $project_name -b $tot_bs -j 128 \
            --frame_num 16 --input_size 128   \
            --opts \
            num_cond_frames 5 \
